...
class CategoriesSyncAdapter extends BaseV2SyncAdapter {
  CategoriesSyncAdapter({super.databaseService});

  Future<V2SyncResult> sync(V2SyncContext context) async {
    final envelope = await buildClient(
      context,
    ).getEnvelope('api/v2/pos-categories');
    final rows = V2SyncUtils.asMapList(
      envelope['categories'] ?? envelope['data'],
    );
    var upsertedCount = 0;
    ...
    await databaseService.transaction((txn) async {
      ...
      for (final row in rows) {
        ...
        await databaseService.upsertByUnique(
          ...
          insertValues: <String, Object?>{
            'tenant_id': tenantId,
            'remote_id': remoteId,
            'code': V2SyncUtils.asString(
              row['commondity_code'] ?? row['commodity_code'],
            ),
            'name': V2SyncUtils.asString(
              row['commondity_name'] ?? row['commodity_name'] ?? row['name'],
            ),
            'brand_name': V2SyncUtils.asString(row['brand']),
      ...
      await touchCheckpoint(
        txn,
        tenantId,
        endpointName: 'pos-categories',
        scopeKey: 'default',
        notes: 'Category master synced from api/v2/pos-categories.',
      );
    });
